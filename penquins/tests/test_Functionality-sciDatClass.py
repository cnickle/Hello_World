...
def test_subset():
    ...
    data = sciData('', fun.linear, rawdat = dat)
    data.subsetData([0,50])
    print(len(data.workingdat['X']))
    assert len(data.workingdat['X']) < 55

def test_returnDataFunc(fName = 'tests\\Data\\2H-c-2F (amps).txt'):
    par = {
        'gammaL'  : 0.0003,
        'gammaR'  : 0.45,
        'deltaE1' : 0.6,
...
def test_returnDataFunc2(fName = 'tests\\Data\\2H-c-2F (amps).txt'):
    par = {
        'gammaL'  : 0.000268,
        'gammaR'  : 0.464514,
        'deltaE1' : 0.574295,
...
def test_plot(fName = 'tests\\Data\\2H-c-2F (amps).txt'):
    par = {
        'gammaL'  : 0.000268,
        'gammaR'  : 0.464514,
        'deltaE1' : 0.574295,
...
def test_plot_InitParandScale(fName = 'tests\\Data\\2H-c-2F (amps).txt'):
    par = {
        'gammaL'  : 0.000268,
        'gammaR'  : 0.464514,
        'deltaE1' : 0.574295,
...
def test_plot_fit(fName = 'tests\\Data\\2H-c-2F (amps).txt'):
...
def test_plot_FitScaleParSave(fName = 'tests\\Data\\2H-c-2F (amps).txt'):
    bnds = {
        'gammaL'  : [0,1],
        'gammaR'  : [0,1],
        'deltaE1' : [0,1.5],
